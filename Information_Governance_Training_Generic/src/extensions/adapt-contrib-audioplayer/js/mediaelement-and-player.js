...
mejs.HtmlMediaElementShim = {
    ...
    if (options.customError) {
      ...
      errorContainer.innerHTML = (poster !== '') ?
        ...
        '<a href="' + playback.url + '" tabindex="-1"><span>' + mejs.i18n.t('Download File') + '</span></a>';
    }
 ...
 *
 *
 * Modified by Łukasz 'Severiaan' Grela
 * added resetSize and rebuildtracks to MEP, cleartracks to the MEP tracks feature
 * version 2.13.2(0.2.2)
 */
	...
	mejs.MediaElementPlayer.prototype = {
		...
		init: function() {
			...
			if ((mf.isiPad && t.options.iPadUseNativeControls) || (mf.isiPhone && t.options.iPhoneUseNativeControls)) {
				...
				// build container
				t.container =
					$('<div id="' + t.id + '" class="mejs-container ' + (mejs.MediaFeatures.svg ? 'svg' : 'no-svg') + '" tabindex="-1">'+
		...
		setControlsSize: function() {
			...
			if (railWidth === 0 || !railWidth) {
				...
				railWidth = Math.floor(t.controls.width() - usedWidth - (rail.outerWidth(true) - rail.width())) - 1;//update for audio player time control issue
			}
        ...
        resetPlayerState: function () {
            ...
            if (!t.options.loop){
                var playLayer = t.layers.find('.mejs-overlay-play');
                    playLayer.show();
                var play = t.controls.find('.mejs-playpause-button');
        ...
        removeEventListener: function (eventName, callback) {
            var t = this;
            if (!eventName) { t.events = {}; return true; }
            var callbacks = t.events[eventName];
            if (!callbacks) return true;
            if (!callback) { t.events[eventName] = []; return true; }
            for (i = 0; i < callbacks.length; i++) {
                if (callbacks[i] === callback) {
                    t.events[eventName].splice(i, 1);
                    return true;
                }
            }
            return false;
...
(function($) {
	...
	$.extend(MediaElementPlayer.prototype, {
		...
		setTrack: function(lang, withoutEvent){
            ...
            //dispatch tracksChanged event
            var TRACKS_CHANGED = 'tracksChanged';
            var event = {type:TRACKS_CHANGED, target:t.media, 'lang':lang};
            if(!withoutEvent)
                t.dispatchEvent(event.type, event);
