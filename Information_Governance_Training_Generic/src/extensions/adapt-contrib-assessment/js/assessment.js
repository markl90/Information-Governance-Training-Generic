    ...
    */
    ...
    //Private functions
        ...
        _checkAssessmentsComplete: function() {
            ...
            console.log('_checkAssessmentsComplete states: ', states);
            ...
            console.log('_checkAssessmentsComplete this._assessments.length: ', this._assessments.length);
            console.log('_checkAssessmentsComplete assessmentStates - ', assessmentStates);

            if(Adapt.course.get("_assessment")._passWithAnyScore) {
                //TODO
            } else {
        ...
        _postScoreToLms: function() {
            ...
            console.log('_postScoreToLms: completionState - ', completionState);
    ...
    //Public functions
        ...
        getState: function() {
            ...
            var assessmentsComplete = 0, isComplete, scoreAsPercent;
            if(Adapt.course.get("_assessment")._passWithAnyScore) {
                //TODO
                var scores = [], maxScores = [];
                for (var id in states) {
                    var state = states[id];
                    if (!state.includeInTotalScore) continue;
                    ...
                    console.log('getState state.maxScore', state.maxScore);
                    console.log('getState state.assessmentWeight', state.assessmentWeight);
                    //score += state.score / state.assessmentWeight;
                    scores.push(state.score);
                    maxScores.push(state.maxScore);
                    isPass = isPass === false ? false : state.isPass;
                }

                //isComplete = assessmentsComplete == totalAssessments;
                
                console.log('getState scores - ' , scores + 'maxScores - ', maxScores);
                var _nScore = this.getSubtractiveScore(scores);
                var _nMaxScore = this.getSubtractiveMaxScore(maxScores);
                console.log('getState _nScore', _nScore);
                console.log('getState maxScore', _nMaxScore);
                scoreAsPercent = Math.round((_nScore / _nMaxScore) * 100);
                isComplete = scoreAsPercent >= assessmentsConfig._scoreToPass;

                console.log('getState scoreAsPercent', scoreAsPercent);
                console.log('getState isComplete', isComplete);
            } else {
               for (var id in states) {
                    var state = states[id];
                    if (!state.includeInTotalScore) continue;
        ...
        getSubtractiveScore:function(p_aScores) {
            console.log('getSubtractiveScore - p_aScores ' ,p_aScores);
            for (var i = 0; i < p_aScores.length; i++) {
                if(p_aScores[i] == undefined) {
                    p_aScores.splice(i,1)
                }
            }
            console.log('getSubtractiveScore - _.max(p_aScores): ' , _.max(p_aScores));
            return _.max(p_aScores);
        },
        getSubtractiveMaxScore:function(p_aScores) {
            console.log('getSubtractiveMaxScore - p_aScores ' ,p_aScores);
            for (var i = 0; i < p_aScores.length; i++) {
                //if(p_aScores[i] == NaN) {
                if(isNaN(p_aScores[i])) {
                    p_aScores.splice(i,1);
                } else {

                }
            }
            console.log('getSubtractiveMaxScore - p_aScores[0]: ' , p_aScores[0]);
            return p_aScores[0];
        }
