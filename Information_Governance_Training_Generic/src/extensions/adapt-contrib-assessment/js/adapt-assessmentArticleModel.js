define([
    ...
    //Private functions
        ...
        _setupAssessmentData: function(force) {
            ...
            this._currentQuestionComponents = this.findDescendants('components').where({_isQuestionType: true});
            this._currentQuestionBlock = this.findDescendants('blocks').where({ _isQuestionType: true });
            console.log("AssessmentModel", "_setupAssessmentData", "this._currentQuestionComponents", this._currentQuestionComponents);


            //number questions
            if (assessmentConfig._questions.hasOwnProperty("_autoNumberTitle") && assessmentConfig._questions._autoNumberTitle._isEnabled) {
                console.log("creating dynamic block title")
                var autoTitleConf = assessmentConfig._questions._autoNumberTitle;
                if (!autoTitleConf.hasOwnProperty("pattern")) {
                    autoTitleConf.pattern = "%current% / %total%";
                }
                var title = autoTitleConf.pattern.replace("%total%", this._currentQuestionComponents.length);
                var i = 1;
                _.each(this._currentQuestionComponents, function(model) {
                    var qt = title.replace("%current%", i);
                    console.log("AssessmentModel", "_setupAssessmentData", i, model, qt);
                    model.set({
                        "title": qt,
                        "displayTitle": qt
                        // "topTitle": qt
        ...
        _setupRandomisedAssessment: function() {
            var assessmentConfig = this.getConfig(),
                randomisationModel = assessmentConfig._randomisation,
                blockModels = this.getChildren().models;
            
            console.log('>>>>>>> blockModels - ', blockModels);
            console.log('>>>>>>> blockModels[0] - ', blockModels[0]);
            console.log('>>>>>>> blockModels[0]._previousAttributes - ', blockModels[0]._previousAttributes);
            console.log('>>>>>>> blockModels[0]._previousAttributes._id - ', blockModels[0]._previousAttributes._id);

            /*QA1509: Sort quiz and push titles between */
            var quizBlockModels = [],
                titleBlockModels = [];

            for (var i = 0; i < blockModels.length; i++) {
                if(blockModels[i]._previousAttributes._id.indexOf('tb')>=0) {
                    titleBlockModels.push(blockModels[i]);
                } else {
                    quizBlockModels.push(blockModels[i]);
                }
            }
            
            console.log('>>>>>>> quizBlockModels - ', quizBlockModels);
            console.log('>>>>>>> titleBlockModels - ', titleBlockModels);

            var questionModels = _.shuffle(quizBlockModels),
                newBlockModels = []; /*QA1509*/

            
            for (var i = 0; i < blockModels.length; i++) {
                newBlockModels.push(titleBlockModels[i]);
                newBlockModels.push(questionModels[i]);
            }
            ...
            newBlockModels = newBlockModels.slice(0, randomisationModel._blockCount);
            return newBlockModels;
            /*QA1509*/
        },
    ...
    //Public Functions
        ...
        getReviewList: function() {
            ...
            for (var i = 0, l = questionComponents.length; i < l; i++) {
                var question = questionComponents[i];
                reviewList._id[i] = question.get('_id');
                reviewList._sectionId[i] = question.get('_sectionId');
                reviewList._isCorrect[i] = question.get('_isCorrect');
                reviewList._questionWeight[i] = question.get('_questionWeight');
            }
            return reviewList;
        }
    };
