define(function(require) {
    ...
    var Accordion = ComponentView.extend({
        ...
        toggleItem: function(event) {
            ...
            var itemIndex = $(event.currentTarget.parentNode).index();
            var $toggleButton = $(event.currentTarget);
            var $accordionItem = $toggleButton.parent('.accordion-item');
            var isCurrentlyExpanded = $toggleButton.hasClass('selected');
            // for (i = 0; i < $(".accordion-item-graphic").length; i++) {
            //     $("#itemImage" + i).css("display", "none");
            // }
            // $(".accordion-cover-graphic").css("display", "none");
            // $("#itemImage" + itemIndex).css("display", "block");
        ...
        checkCompletionStatus: function() {
            //console.log('VisitedItems: ' + this.getVisitedItems().length + " / " + this.model.get('_items').length);
