define(function(require) {
    ...
    var Mcq = QuestionView.extend({
        ...
        setupQuestion: function() {
            ...
            this.listenTo(Adapt, 'notify:cancelled', this.onFeedbackClosed);
        ...
        onQuestionRendered: function() {
            ...
            var _aAlpha = ['A','B','C','D','E','F','G','H','I','J'];
            this.$('.mcq-item').each(function(index, item){
                $(this).find('.mcq-item-inner-index').html(_aAlpha[index]);
            });
        ...
        toggleItemSelected:function(item, clickEvent) {
            ...
            if(selected) {
                if(this.model.get('_selectable') === 1){
                    if(!$(clickEvent.currentTarget).closest('.mcq-component').hasClass('submitted')){
                        if(this.model.get('_items')[itemIndex]._type=="intermediate")
                            $(clickEvent.currentTarget).closest('.mcq-component').addClass('isIntermediate');
                        else
                            $(clickEvent.currentTarget).closest('.mcq-component').removeClass('isIntermediate');
                    }
        ...
        canSubmit: function() {
            ...
            _.each(this.model.get('_items'), function(item) {
                if (item._isSelected) {
                    if(item._type== "intermediate") intermediateSelected = true;
                    count++;
                }
            }, this);
            var compID = this.model.get("_id");
            console.log('canSubmit, count: ', count, ', intermediateSelected: ', intermediateSelected, ", $(('.mcq-component.'+compID)).hasClass('intermediate-removed'): ", $(('.mcq-component.'+compID)).hasClass('intermediate-removed'));
            if(count==1 && intermediateSelected && $(('.mcq-component.'+compID)).hasClass('intermediate-removed')) return false;

            //
            if(this.model.get('_correct') != undefined) {
               if(this.model.get('_items').length == this.model.get('_correct')) {
                    return (count > 0) ? true : false;
                } else {
                    return (count == this.model.get('_correct')) ? true : false;
                } 
            } else {
                return (count > 0) ? true : false;
            }
        ...
        setupFeedback: function() {
            ...
            //if(this.isAudio) {
                ...
                setTimeout(function(){
                    ...
                    Adapt.audio._$audioElement[0].pause();
                },500);
                */
                
                //var currentIndex = $itemEl.index();
                //var currentAudio = this.model.get('_items')[currentIndex].audio;
                //var currentAudio = this.model.get('_feedback').audio;
                
                ///this.$el.parents().find('audio').attr('src','');
                //this.audioPlayer.pause();
                //this.$el.parents().find('audio').attr('src',currentAudio);
            //}

            var groupID = this.model.get('_groupID');
            var screenID = this.model.get('_id');
            if(groupID && groupID.length>0){
                var result = this.model.get('_isCorrect')?1:0;
                Adapt.BranchingRecordKeeping.updateGroupRecord(groupID, screenID, result);
            }
        },
        ...
        onFeedbackClosed: function() {
            //if(this.isAudio) this.muteAudio();
        },
        ...
        muteAudio: function() {
            ...
            //},500);
            /*
            this.audioPlayer = Adapt.audio._$audioElement[0];
            this.audioPlayer.pause();
